...
%             dy = [v;cnb'*(acc - ba);omega/2;zeros(6,1)];
        ...
        function [state,predErrorState] = imu2state(o,acc_nose,gyro_noise,gps_noise,state0,errorstate0,tspan,step,isODE45)
            %acc和gyro分别为加表陀螺仪数据
            %state0：
            %初始状态，1-3是xyz位置，4-6是xyz速度，7-10是四元数，11-13是载体系下的加速度偏差，14-16是载体系下陀螺仪偏差
            %errorstate0：   偏差初始状态
            %Qc0：           误差方差阵
            %tspan：         时间
            %step：          步长
            ...
            N = length(tspan);
            % state是状态估计值，预测环节得到先验值，校正过程减去卡尔曼得到的偏差后成为最优估计值
            state = zeros(N,length(state0));
            predErrorState=zeros(N,length(errorstate0));  % 先验GPS与inv的偏差估计值
            ...
            for i = 1:N - 1
                % 下面这部分是利用惯导的数据来预测，持续到得到predP{i+1}
                ...
                % 预测过程是非线性的，需要求离散化雅克比
                [Fd,Gc,Fc]=o.getExponentMatFd(predCbn,step,acc_nose(i,:)-state(i+1,14:16),gyro_noise(i,:)-state(i+1,11:13));%acc取i行还是i+1行？

%                 predErrorState(i+1,:)=predErrorState(i,:)*Fd';  %误差状态预测
                % predErrorState维度是15的原因是相比state少了四元数的第一个自由度，因为有上面的归一化方程限制，state没有做归一化限制
...
%             Qd=step*Q_hat+step^2*(0.5*Fc*Q_hat+Q_hat*0.5*Fc');
        ...
        function [postErrorState,postP]=MeasurementUpdate(o,error_gps_noise,predP,predErrorState)
            ...
            H=[eye(3) zeros(3,12)];                 %量测矩阵（状态到观测的转移矩阵）
